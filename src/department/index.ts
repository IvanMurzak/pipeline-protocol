...
/**
 * The `department.*` wire vocabulary — assembled. Mirrors `../wire/
...
// ── Re-export every schema, type, and constant from the department submodules ──
export * from "./task.js";
...
/** Every RUNNER → CLOUD (agent→server) department variant, keyed by `type`. */
...
/** Every CLOUD → RUNNER (server→agent) department variant, keyed by `type`. */
